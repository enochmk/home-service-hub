...
export async function verifyJWT(
  ...
  if (!authorization)
    return next(
      new createHttpError.Unauthorized('Authorization header is required'),
    );

  if (!authorization.startsWith('Bearer'))
    return next(
      new createHttpError.BadRequest('Invalid authorization header format'),
    );
...
export async function validateCurrentUser(
  ...
  if (!userId)
    return next(
      new createHttpError.Unauthorized('Invalid token. Please login again'),
    );
  ...
  logger.verbose(`Checking if user: ${user?.email} should update password...`, {
    user,
  });
  if (user?.shouldUpdatePassword) {
    return next(
      new createHttpError.Unauthorized(
        'Please update your password before you can proceed',
      ),
    );
  }
...
export async function shouldUpdatePassword(
  req: Request,
  res: Response,
  next: NextFunction,
) {
...
export async function checkUserCompanyAssociation(
  req: Request,
  res: Response,
  next: NextFunction,
) {
